...
for comm_round in range(comms_round):
    ...
    def test_model(X_test, Y_test,  model, comm_round):
        ...
        logits = model.predict(X_test)
        loss = cce(Y_test, logits)
        acc = accuracy_score(tf.argmax(logits, axis=1),
...
def non_iid_x(image_list, label_list, x=1, num_intraclass_clients=10):
    ...
    sub_lab_list = [unique_labels[i:i + x]
                    for i in range(0, len(unique_labels), x)]

    for item in sub_lab_list:
        class_data = [(image, label) for (image, label) in zip(
            image_list, label_list) if label in item]
        ...
        intraclass_clients = create_clients(
            list(images), list(labels), num_intraclass_clients, initial)
